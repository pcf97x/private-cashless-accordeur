...
<div class="max-w-3xl mx-auto">
    ...
    <div class="card p-6 mb-6">
        <h3 class="text-xs font-bold uppercase tracking-widest text-gray-400 pb-3 mb-3 border-b border-gray-100">Factures</h3>
        <div class="space-y-2">
            @foreach($reservation->invoices as $inv)
            <a href="{{ route('admin.invoices.show', $inv) }}" class="flex items-center justify-between p-3 rounded-lg hover:bg-accordeur-50/50 transition-colors">
                <div>
                    <span class="font-semibold text-accordeur-600">{{ $inv->invoice_number }}</span>
                    <span class="text-sm text-gray-500 ml-2">{{ $inv->client_name }}</span>
                </div>
                <span class="text-sm font-semibold">{{ number_format($inv->total_ht, 2, ',', ' ') }} &euro;</span>
            </a>
            @endforeach
        </div>
    </div>
    ...
    <div class="card p-6">
        ...
        <div class="flex flex-wrap gap-3">
            <a href="{{ route('admin.invoices.create', ['reservation_ids' => [$reservation->id]]) }}" class="btn-outline">
