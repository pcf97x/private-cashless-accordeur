...
<div class="max-w-2xl mx-auto">
    ...
    <div class="page-header">
        ...
        <p>Définissez une plage horaire de réservation</p>
    </div>

    <div class="card p-6 sm:p-8">
        <form method="POST" action="{{ route('admin.time-slots.store') }}" class="space-y-5">
            @csrf

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label for="code" class="form-label">Code</label>
                    <input type="text" name="code" id="code" required class="form-input" placeholder="Ex: MATIN" value="{{ old('code') }}">
                    @error('code') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="label" class="form-label">Libellé</label>
                    <input type="text" name="label" id="label" required class="form-input" placeholder="Ex: Matinée" value="{{ old('label') }}">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="time" name="start_time" id="start_time" required class="form-input" value="{{ old('start_time') }}">
                    @error('start_time') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="end_time" class="form-label">Heure de fin</label>
                    <input type="time" name="end_time" id="end_time" required class="form-input" value="{{ old('end_time') }}">
            ...
            <div>
                ...
                <input type="number" name="order_index" id="order_index" min="0" class="form-input" placeholder="0" value="{{ old('order_index', 0) }}">
            ...
            <div class="flex items-center gap-3 pt-4 border-t border-gray-100">
                <button type="submit" class="btn-primary">Créer le créneau</button>
