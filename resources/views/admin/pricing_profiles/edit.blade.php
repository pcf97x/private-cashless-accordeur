...
<div class="max-w-2xl mx-auto">
    ...
    <div class="page-header">
        <h1>Modifier : {{ $profile->label }}</h1>
        <p>Mettez à jour ce profil tarifaire</p>
    </div>

    <div class="card p-6 sm:p-8">
        <form method="POST" action="{{ route('admin.pricing-profiles.update', $profile) }}" class="space-y-5">
            @csrf
            @method('PUT')

            <div>
                <label for="label" class="form-label">Libellé</label>
                <input type="text" name="label" id="label" required class="form-input" value="{{ old('label', $profile->label) }}">
            ...
            <div>
                ...
                <input type="text" name="code" id="code" required class="form-input font-mono uppercase" value="{{ old('code', $profile->code) }}">
                @error('code') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>

            <div class="flex items-center gap-3">
                <input type="checkbox" name="active" id="active" value="1" {{ old('active', $profile->active) ? 'checked' : '' }} class="rounded border-gray-300 text-accordeur-500 focus:ring-accordeur-500">
